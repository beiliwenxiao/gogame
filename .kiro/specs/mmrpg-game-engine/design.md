# 技术设计文档：MMRPG 游戏引擎 (yijian18-server)

## 概述

yijian18-server 是轻量级 MMRPG 纯游戏后端引擎，核心卖点为千人同屏战斗。引擎采用 ECS（Entity-Component-System）架构，支持帧同步（Lockstep）与状态同步（State Sync）双模式，提供层次化地图系统、装备系统、战斗系统等核心模块。
